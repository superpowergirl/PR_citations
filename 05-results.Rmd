...
plot_ly(
          ...
          # Hover text:
              hoverinfo = 'text',
          text = ~paste(Year)
      ) %>% layout(title = "Number of Publications by Year")
...
plot_ly(
          ...
          # Hover text:
              hoverinfo = 'text',
          text = ~paste(Year)
      ) %>% layout(title = "Number of Citatoins by Published Year")
...
We are interested in studying the papers published in 2008 when number of citations reached record high.
...
The parallel coordinates showed us some interesting trends. First, in contrast to general belief, number of authors is negatively correlated with paper length. Second, most journal categories have author counts spread from 1 to 25 while two categories - RMP and PRSTPER have relatively fewer authors.
...
Furthermore, we would like to see for the publications in 2008, how the number of citations varies across journal categories.

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.width=13}
recent_mo <- as.data.frame(recent)

recent_mo$n <- cut(recent_mo$n, breaks = c(-Inf, 6, 15, Inf), labels = c('low', 'medium','high'), right = FALSE)
